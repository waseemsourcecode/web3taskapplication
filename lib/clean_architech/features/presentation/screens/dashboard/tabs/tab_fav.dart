...
class TabFav extends StatelessWidget{
  const TabFav({super.key});

  @override
  Widget build(BuildContext context) {
  
    return Column(children: [
      const Center(child: CustomLabel(text: "Favourite Products", forceColor: Colors.black, type: TextStyleType.f22_600,),),
          dimenVertical(30),
...
BlocProvider.of<CubitProduct>(context).loadAllProducts();
  ...
  return const WidgetLoading();
    ...
    BlocProvider.of<CubitProduct>(context).loadFavProduct(state.product);
}
else if(state is StateProductLoadedFav){
  onConsole("HERE OK");
return     Expanded(
